...
""""""""""""""""""""""""""""""
def test_generate_x509cert_and_key_and_store_to_disk(path_to_dir, input_password):
...
def test_sign_and_verify(path_to_key_file, input_password, message):
    ...
    print("\nRSA PUB KEY: \n")
    print_rsa_pub_key(rsa_priv_key.public_key())"""
...
def test_sign_from_file_verify_from_cert_file(path_to_key_file, input_password, 
    ...
    print_x509cert(x509cert)
    print("\nRSA PUB KEY: \n")
    print_rsa_pub_key(x509cert.public_key())"""
...
def verify_signature_from_cert_file(path_to_cert_file, message, signature):
    #load certificate and convert it to a PEM byte data
    x509cert = x509_load_certificate_from_file(path_to_cert_file)
    """print ("\nX509 CERTIFICATE: \n")
    print_x509cert(x509cert)
...
def test_verify_signature_from_cert_file_loop(path_to_cert_file, message, proofs):
...
def crypto_suite_test(tests_to_execute, path_to_dir, path_to_key_file, input_password, 
    path_to_cert_file, message, proofs):
    
    if tests_to_execute[0]:
        print("CRYPTO TEST 1: Generate x509 certificate and RSA Key and store to disk:\n")
        test_generate_x509cert_and_key_and_store_to_disk(path_to_dir, input_password)
    if tests_to_execute[1]:
        ...
        test_sign_and_verify(path_to_key_file, input_password, message)
